...
Usage example:
    python src/biomedclip_gloria_pretrain.py \\
        --dataset data/internal_dataset/dataset_full.json \\
...
from biomedclip_gloria.loss.gloria import gloria_combined_loss
...
def _encode_image_with_patches(model: nn.Module, images: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """Pooled image embedding + patch tokens (CLS dropped), from a single ViT forward pass.

    Equivalent to (model.encode_image(images), patch_tokens) but without running
    the ViT trunk twice: trunk.forward_head(tokens) reproduces exactly what
    TimmModel.forward's `self.trunk(x)` computes internally.
    """
    tokens       = model.visual.trunk.forward_features(images)  # [B, 197, 768], CLS at index 0
    pooled       = model.visual.trunk.forward_head(tokens)
    image_feat   = model.visual.head(pooled)
    patch_tokens = tokens[:, 1:, :]                              # [B, 196, 768]
    return image_feat, patch_tokens
...
def train_one_epoch(
    ...
    """Run one training epoch (GLoRIA global+local loss) and return the average batch loss."""
    model.train()
    total_loss = 0.0
    ...
    try:
        for batch in pbar:
            ...
            with torch.autocast(device_type=device.type, dtype=torch.float16):
                image_feat, patch_tokens = _encode_image_with_patches(model, images)
                text_feat, word_tokens   = model.text(texts)  # output_tokens=True -> (pooled, [B,256,768])
                loss, _ = gloria_combined_loss(
                    image_feat, text_feat, patch_tokens, word_tokens, texts,
                    pad_token_id=special_ids["pad"],
                    cls_token_id=special_ids["cls"],
                    sep_token_id=special_ids["sep"],
                    temp1=gloria_cfg["temp1"], temp2=gloria_cfg["temp2"], temp3=gloria_cfg["temp3"],
                    local_loss_weight=gloria_cfg["local_loss_weight"],
                    global_loss_weight=gloria_cfg["global_loss_weight"],
                )
        ...
        # evaluate()/evaluate_retrieval() call model.encode_text(), which breaks
        # if output_tokens is left True (HFTextEncoder would return a tuple).
        model.text.output_tokens = False
...
def parse_args(argv=None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="BiomedCLIP pretraining with LoRA on the ViT encoder, using GLoRIA's global+local loss."
    )
    # ── Data / splits ────────────────────────────────────────────────────────
    ...
    # ── GLoRIA loss hyperparameters (defaults match sbatch/gloria/run_gloria_pretrain.sh) ──
    parser.add_argument("--temp1", type=float, default=5.0,
                        help="Word-region attention sharpness (default: %(default)s)")
    parser.add_argument("--temp2", type=float, default=4.0,
                        help="Word-region similarity scaling before log-sum-exp aggregation (default: %(default)s)")
    parser.add_argument("--temp3", type=float, default=15.0,
                        help="Final batch softmax temperature, shared by local and global loss (default: %(default)s)")
    parser.add_argument("--local_loss_weight", type=float, default=1.0,
                        help="Weight of the local (word-region) loss term (default: %(default)s)")
    parser.add_argument("--global_loss_weight", type=float, default=1.5,
                        help="Weight of the global (sentence-image) loss term (default: %(default)s)")
    ...
    parser.add_argument("--wandb_project", default="biomedclip-gloria-pretrain",
...
def _apply_sweep_config(args: argparse.Namespace) -> None:
    ...
    cfg = wandb.config
    for key in ("lora_layers", "lora_r", "lr_lora", "lr_proj", "weight_decay", "batch_size",
                "unfreeze_blocks", "lr_blocks", "temp1", "temp2", "temp3",
...
def _make_run_dir(args: argparse.Namespace) -> tuple[str, Path]:
    ...
    run_dir = Path(args.out_dir) / "biomedclip_gloria_pretrain" / run_name
...
def _load_model(
...
) -> tuple[nn.Module, object, object, object, dict]:
    """Load BiomedCLIP and apply LoRA or partial-unfreeze adaptation.

    Returns (model, tokenizer, preprocess_val, augment_transform, special_ids).
    """
    ...
    return model, tokenizer, preprocess_val, augment_transform, special_ids
...
def _train_fold(
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss = train_one_epoch(
            model, train_loader, optimizer, scaler, device, trainable_params, gloria_cfg, special_ids)
        # evaluate() computes the symmetric CLIP val loss (not GLoRIA's loss) —
        # a stable, apples-to-apples monitoring signal shared with the plain
        # BiomedCLIP baseline; checkpoint selection below is driven by
        # retrieval R@1 either way.
        ...
        print(
            ...
            f"train={train_loss:.4f} | val_clip_loss={val_loss:.4f} | "
...
def _run_fold(
    ...
    model, tokenizer, preprocess_val, augment_transform, special_ids = _load_model(args, device)
    ...
    _train_fold(
        ...
        optimizer, trainable_params, scheduler, scaler, special_ids, use_wandb, this_run_dir, device,
    )
