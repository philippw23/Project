"""Evaluate a *saved* ImageNet-ViT downstream head — no training, no hyperparameter flags.
...
from imagenet_img.data.transforms import build_val_transform
from imagenet_img.models.encoders import build_encoder
from imagenet_img.train.downstream import evaluate_cached


def parse_args(argv=None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate a saved ImageNet-ViT downstream head (no training).")
    p.add_argument("--head_checkpoint", required=True,
                   help="Trained head .pt (best_checkpoint.pt) — carries its own config.")
...
def _load_config(head_ckpt: dict, cli: argparse.Namespace) -> argparse.Namespace:
    if "args" not in head_ckpt:
        raise SystemExit(
            "This head checkpoint has no embedded config. Retrain with the updated downstream.py."
        )
...
def main(cli: argparse.Namespace) -> dict:
    ...
    def _score(samples: list[dict]) -> tuple[float, np.ndarray, np.ndarray, np.ndarray]:
        ...
        loss, _, preds, labels, probs = evaluate_cached(mlp, loader, criterion, device)
