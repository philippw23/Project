...
def load_pretrained_weights(
    ...
    # Older local revisions flattened the chunked upstream block layout and
    # renamed SwiGLU weights. The exact CheXFound model uses upstream keys
    # directly, so only keep the flattening fallback when the target model is
    # genuinely flat.
    model_keys = set(model.state_dict().keys())
    has_chunked_target = any(k.startswith("blocks.0.0.") for k in model_keys)
    has_flat_target = any(k.startswith("blocks.0.") and not k.startswith("blocks.0.0.") for k in model_keys)
    if has_flat_target and not has_chunked_target:
        import re
        staged = re.compile(r"^blocks\.\d+\.(\d+)\.(.+)$")
        remapped = {}
        for k, v in state_dict.items():
            m = staged.match(k)
            if m:
                remapped[f"blocks.{m.group(1)}.{m.group(2)}"] = v
            else:
                remapped[k] = v
        state_dict = remapped
