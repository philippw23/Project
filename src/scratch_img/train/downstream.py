...
def main(args: argparse.Namespace) -> None:
    ...
    eval_metrics: dict = {}
    if args.eval_test:
        test_loss, _, test_preds, test_labels = evaluate(encoder, mlp, test_loader,
                                                         criterion, device)
        ...
        if args.btxrd_manifest:
            print(f"Loading BTXRD samples from {args.btxrd_manifest}...")
            btxrd_samples = load_btxrd_samples(args.btxrd_manifest)
            btxrd_loader, n_btxrd = build_downstream_loader(
            ...
            btxrd_loss, _, btxrd_preds, btxrd_labels = evaluate(
                encoder, mlp, btxrd_loader, criterion, device)
            eval_metrics.update(report_eval(
