...
# Results go to results/scratch_img/ and results/scratch_img_text/ respectively.
# Splits are generated by BASELINE 1 and saved to results/scratch_img/splits.json,
# then reused by BASELINE 2.
...
python $home_dir/Project/src/scratch_img_text_downstream.py \
    --encoder        $ENCODER \
    --english \
    --out_dir        $home_dir/Project/results/scratch_img_text \
    --splits         $home_dir/Project/results/scratch_img/splits.json \
    --overfit_n      8 \
    --epochs         400 \
    --patience       400 \
    --batch_size     8 \
